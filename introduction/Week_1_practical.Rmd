...
ouput: html_document
```
#Concept diagram in R is presented below
![](./diagram.png)
#Concept diagram in R is shown above
...
ouput: the variable y contains the mean of 5 values from x vector
```
```{r}
x <- c(7, 9, 14, 25, 26)
print(x)
...
* What does the following create? 
      ** it creates a vector in R
  * Specifically, what is the of the data it contains?
      ** it contains numeric and character

`c(1, 2, 3, "Hi")`

* Would the following be valid?
    ** No, this script is not valid 
...
```{r}
library(ggplot2)
plot <- qplot(x_axis, y_axis, geom = "point", size = I(3)) +
  labs(x = "My x_axis", y = "My y_axis", title = "My amazing plot of x_axis versus y_axis")
print(plot)
